...
However, we will later show that the specific dinatural transformations we are interested in can compose freely.
...
As they are the only two which make explicit use of the dual operator, this is to be expected.
...
We can deduce that a cycle could form if the conclusion of an $\aiD$ rewrite later became the premise of an $\aiU$.
\begin{figure}[ht]
...
This simple cycle can clearly never occur as the corresponding structure brackets do not match up ($\aiD$ produces a copar while $\aiU$ requires a par).
Instead, a more complex cycle may form if we allow additional transformations inbetween the applications of $\aiU$ and $\aiD$.
Looking back at the first example, we could concievably add an instance of $\aiU$ at the bottom.
This would introduce an additional arrow (dotted), forming a longer cycle (bolded):
...
\caption{A modification of a graph of a well-formed $\sSys$ derivation, with a cycle formed}
\end{figure}

We must now verify that this new graph is not itself the graph of a well-formed derivation.
That is, we must show that there is no well-formed derivation of the following form:
...
REWRITE THE MIDDLE BIT ABOVE AS DERIVATION NOT ELIPSES
