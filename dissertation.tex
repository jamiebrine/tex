...
The key idea underpinning these representations is the Curry-Howard correspondance.
This was first proposed by Haskell Curry in the 1940s, then explicitly formulated by William Alvin Howard in the 1960s.
A typeset version of Howard's original hand-written notes have been published \cite{howard1980formulae}, or for a more modern and detailed explaination the reader is referred to a Master's thesis by Husna Farooqui \cite{farooqui2021curry}
It can be distilled to the following simple phrase:

\begin{center}
"Propositions are types, and proofs are programs"
\end{center}

More formally, it is a framework in which constructing a proof represents the execution of a program.
The type of the executed program is precisely the type which the proof's conclusion represents.
It allows for programs to be more rigorously studied, particularly with regard to their correctness and termination; if the proof is valid in the given system, so is the program it represents.

This idea has been further enriched with the advent of deep inference logic. 
